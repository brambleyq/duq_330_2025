...
def pair_up(first_df:pd.DataFrame,first_id_col:str,second_df:pd.DataFrame,second_id_col:str,fastText:fasttext.FastText._FastText,nearest_neighbors:int = 1000) -> pd.DataFrame:
    ...
    first_df['surname_vec'] = first_df['forename'].apply(fastText.get_sentence_vector)
    second_df['surname_vec'] = second_df['surname'].apply(fastText.get_sentence_vector)


    
    pairs = []
    second_vectors = np.array([array.tolist() for array in second_df['surname_vec']])
    ...
    for _,row_a in first_df.iterrows():
        closest_indexes,_ = p.knn_query(row_a['surname_vec'],nearest_neighbors)
...
def name_distance(paired_dataset:pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        ...
        surname_a, surname_b, forename_a, forename_b

    Returns:
        pd.DataFrame: adds a columns for the 
        distances surname_distance and forename_distance
    """
...
if __name__ == '__main__':
    ...
    start = time.time()
    paired = pair_up(assignee_df,'patent_id',npi_df,'npi',ft_model)
